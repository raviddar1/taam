...
// ---- אנימציה משותפת ----
  ...
  function drawScene3(p) {
    ...
    // [1] triTri — 2 purple triangles (רביע), right slides first
    { const _tt = anim003[1];
      if(_tt.phase>=1){
        ...
        const slideAmt = 60;
        df(p, 170,150,232);
        p.triangle(tt.x+tw3+p.lerp(slideAmt,0,eR), tt.y, tt.x+tw3*2+p.lerp(slideAmt,0,eR), tt.y-th3, tt.x+tw3*2+p.lerp(slideAmt,0,eR), tt.y+th3);
        p.triangle(tt.x+p.lerp(slideAmt*0.5,0,eL), tt.y, tt.x+tw3+p.lerp(slideAmt*0.5,0,eL), tt.y-th3, tt.x+tw3+p.lerp(slideAmt*0.5,0,eL), tt.y+th3);
      } else {
        df(p, 170,150,232);
        p.triangle(tt.x,     tt.y, tt.x+tw3,   tt.y-th3, tt.x+tw3,   tt.y+th3);
        p.triangle(tt.x+tw3, tt.y, tt.x+tw3*2, tt.y-th3, tt.x+tw3*2, tt.y+th3);
      } }
  ...
  function playVerse013() {
    ...
    for(var _i=0;_i<10;_i++){(function(idx){
      timers013.push(setTimeout(function(){anim013[idx].phase=1;anim013[idx].t=0;anim013[idx].startTime=performance.now();},VERSE013_DELAYS[idx]));
    })(_i);}
  }
  ...
  function playVerse003(){stopVerse003();for(var _i=0;_i<9;_i++){(function(idx){timers003.push(setTimeout(function(){anim003[idx].phase=1;anim003[idx].t=0;anim003[idx].startTime=performance.now();},VERSE003_DELAYS[idx]));})(_i);}}
  ...
  function playVerse004(){stopVerse004();for(var _i=0;_i<8;_i++){(function(idx){timers004.push(setTimeout(function(){anim004[idx].phase=1;anim004[idx].t=0;anim004[idx].startTime=performance.now();},VERSE004_DELAYS[idx]));})(_i);}}
  ...
  function playVerse005(){stopVerse005();for(var _i=0;_i<12;_i++){(function(idx){timers005.push(setTimeout(function(){anim005[idx].phase=1;anim005[idx].t=0;anim005[idx].startTime=performance.now();},VERSE005_DELAYS[idx]));})(_i);}}
  ...
  function playVerse007(){stopVerse007();for(var _i=0;_i<10;_i++){(function(idx){timers007.push(setTimeout(function(){anim007[idx].phase=1;anim007[idx].t=0;anim007[idx].startTime=performance.now();},VERSE007_DELAYS[idx]));})(_i);}}
  ...
  function playVerse008(){stopVerse008();for(var _i=0;_i<15;_i++){(function(idx){timers008.push(setTimeout(function(){anim008[idx].phase=1;anim008[idx].t=0;anim008[idx].startTime=performance.now();},VERSE008_DELAYS[idx]));})(_i);}}
  ...
  function playVerse009(){stopVerse009();for(var _i=0;_i<13;_i++){(function(idx){timers009.push(setTimeout(function(){anim009[idx].phase=1;anim009[idx].t=0;anim009[idx].startTime=performance.now();},VERSE009_DELAYS[idx]));})(_i);}}
  ...
  function playVerse010(){stopVerse010();for(var _i=0;_i<19;_i++){(function(idx){timers010.push(setTimeout(function(){anim010[idx].phase=1;anim010[idx].t=0;anim010[idx].startTime=performance.now();},VERSE010_DELAYS[idx]));})(_i);}}
  ...
  function playVerse011(){stopVerse011();for(var _i=0;_i<11;_i++){(function(idx){timers011.push(setTimeout(function(){anim011[idx].phase=1;anim011[idx].t=0;anim011[idx].startTime=performance.now();},VERSE011_DELAYS[idx]));})(_i);}}
  ...
  function onMidiMsg(msg) {
    ...
    if ((st & 0xF0) === 0x90 && vel > 0) {
      ...
      if(note===_nR&&_nRt==='note'){ flashArrow('pnav-next','arrow-flash-left');  _navPage(-1); return; }
      if(note===_nL&&_nLt==='note'){ flashArrow('pnav-prev','arrow-flash-right'); _navPage(1);  return; }
    ...
    if ((st & 0xF0) === 0xB0) {
      const val = vel / 127;
      if(note===_nR&&_nRt==='cc'){ const on=val>0.5; if(on&&!_nRprev){ flashArrow('pnav-next','arrow-flash-left');  _navPage(-1); } _nRprev=on; return; }
      if(note===_nL&&_nLt==='cc'){ const on=val>0.5; if(on&&!_nLprev){ flashArrow('pnav-prev','arrow-flash-right'); _navPage(1);  } _nLprev=on; return; }
